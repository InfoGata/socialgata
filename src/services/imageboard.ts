...
/**
 * Convert imageboard content to plain text
    ...
    return content
      .map((item: unknown) => {
        ...
        if (typeof item === "object" && item !== null) {
          const obj = item as { type?: string; content?: string };
          if (obj.type === "text" || obj.type === "link") return obj.content || "";
          if (obj.type === "quote") return `>${obj.content || ""}`;
        }
        return "";
      })
      .join(" ");
...
const imageboardThreadToPost = (
  ...
  return {
    ...
    body: firstComment ? contentToText(firstComment.content) : undefined,
...
const imageboardCommentToPost = (
  ...
  const attachmentUrl = attachment ? getAttachmentUrl(attachment) : undefined;

  return {
    apiId: String(comment.id),
    body: contentToText(comment.content),
  ...
  async getComments(request: GetCommentsRequest): Promise<GetCommentsResponse> {
    ...
    try {
      ...
      const post: Post = {
        apiId: String(thread.id),
        title: thread.title || thread.autogeneratedTitle || `Thread #${thread.id}`,
        body: opComment ? contentToText(opComment.content) : undefined,
        publishedDate: thread.createdAt?.toISOString(),
        communityApiId: thread.boardId,
        communityName: `/${thread.boardId}/`,
