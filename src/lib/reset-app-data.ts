 ...
 * and by the time the boundary renders, Dexie and automerge-repo are both
 * holding one that nothing reachable from the fallback can close. So the click
 * records the intent and reloads, and the work happens at the top of main.tsx,
 * before a connection has been opened.
 */

const RESET_FLAG = "socialgata:reset-app-data";

/**
 * Deleted even when `indexedDB.databases()` isn't available to enumerate them
 * (Firefox only shipped it in 126). Keep in sync with src/database.ts and
 * FavoritesRepoProvider.
 */
const KNOWN_DATABASES = ["SocialGataDatabase", "socialgata-favorites"];
...
const databasesToDelete = async (): Promise<string[]> => {
  ...
  // alone — plugin frames run on subdomains, but the extension and any host
  // page can put storage here too.
  try {
    const existing = (await indexedDB.databases?.()) ?? [];
    for (const { name } of existing) {
      if (name?.toLowerCase().startsWith("socialgata")) names.add(name);
    }
  } catch {
