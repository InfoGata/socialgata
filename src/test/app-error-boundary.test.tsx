...
const Boom: React.FC<{ throws: boolean }> = ({ throws }) => {
  if (throws) throw new Error("automerge storage is unavailable");
  return <p>the app</p>;
};

// The boundary is above every provider in main.tsx; i18next is the only thing
...
describe("AppErrorBoundary", () => {
  ...
  it("shows a recoverable fallback instead of unmounting the app", async () => {
    ...
    expect(screen.getByText("SocialGata couldn't start")).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText("automerge storage is unavailable")
    ).toBeInTheDocument();
  });
  ...
  it("re-raises the error so window-level exception capture still sees it", () => {
    ...
    expect(reportError).toHaveBeenCalledWith(
      expect.objectContaining({ message: "automerge storage is unavailable" })
    );
  });
...
describe("app data reset", () => {
  ...
  it("does nothing at startup unless a reset was requested", async () => {
    ...
    expect(window.localStorage.getItem("persist:ui")).toBe("{}");
  });

  it("defers the deletion to the next boot, because the databases are still open", () => {
    requestAppDataReset();

    expect(window.localStorage.getItem("socialgata:reset-app-data")).toBe("1");
    // Deleting here would block on Dexie's and automerge's live connections.
  ...
  it("clears persisted state and the app's databases on the next boot", async () => {
    const open = indexedDB.open("SocialGataDatabase", 1);
    ...
    expect(remaining.map((d) => d.name)).not.toContain("SocialGataDatabase");
  });
});
